...
export async function importBrandWebsiteCsv(opts: { csvPath: string; dryRun: boolean }) {
...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  importBrandWebsiteCsv({ csvPath: path, dryRun })
    .then(({ runId, stats }) => {
      console.log(`Brand website CSV import complete. runId=${runId}`);
